...
export default function ChatInput({ channels, defaultChannelId, isDM, dmChannelId, dmSource }: ChatInputProps) {
  ...
  return (
    <div className="px-2 sm:px-4 pb-4 sm:pb-6 pt-3 sm:pt-4 shrink-0">
      ...
      <div className="rounded-lg bg-oct-surface-raised border border-oct-border">
        {files.length > 0 && (
          <div className="px-4 pt-3 pb-2 flex gap-2 overflow-x-auto border-b border-oct-border">
            {files.map((file, i) => (
              <div key={i} className="relative group/file shrink-0">
                {filePreviewUrls[i] ? (
                  <div className="w-[60px] h-[60px] rounded overflow-hidden bg-oct-surface">
                    <img src={filePreviewUrls[i]!} alt={file.name} className="w-full h-full object-cover" />
                  </div>
                ) : (
                  <div className="w-[60px] h-[60px] rounded bg-oct-surface flex flex-col items-center justify-center gap-0.5 px-1">
                    <FileIcon size={18} className="text-oct-muted" />
                    <span className="text-[8px] text-oct-muted truncate w-full text-center">{file.name.split('.').pop()?.toUpperCase()}</span>
                  </div>
                )}
                <button
                  onClick={() => removeFile(i)}
                  className="absolute -top-1.5 -right-1.5 w-[18px] h-[18px] rounded-full bg-oct-bg border-2 border-oct-border flex items-center justify-center opacity-0 group-hover/file:opacity-100 transition-opacity"
                >
                  <X size={8} className="text-oct-text" />
                </button>
                <div className="mt-0.5 text-[8px] text-oct-muted truncate w-[60px] text-center leading-tight">{file.name}</div>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="flex items-center min-h-[44px]">
          <button
            ...
            className="flex items-center justify-center w-[44px] shrink-0 text-oct-muted hover:text-oct-text transition-colors"
            title="Upload a file"
          >
          ...
          <div className="flex-1 min-w-0">
            <textarea
              ...
              className="w-full bg-transparent text-oct-text text-[15px] leading-[1] pt-[16px] pb-[12px] outline-none placeholder:text-oct-muted resize-none overflow-y-hidden"
              style={{ maxHeight: 144 }}
              rows={1}
              disabled={sending}
            />
              ...
              <div className="relative">
                <button
                  ...
                  className={`flex items-center justify-center w-[34px] h-[34px] rounded transition-colors ${
                    dropdownOpen ? 'text-oct-text' : 'text-oct-muted hover:text-oct-text'
                  }`}
                ...
                {dropdownOpen && (
                  <>
                    ...
                    <div className="absolute bottom-full right-0 mb-2 w-60 max-h-[280px] overflow-y-auto bg-oct-surface rounded-md shadow-xl border border-oct-border z-50 py-1">
                      {Object.entries(groupedChannels).map(([groupName, { channels: chs, isTelegram }]) => (
                        <div key={groupName}>
                          <div className="px-2.5 pt-2 pb-0.5 text-[10px] font-bold uppercase tracking-wider text-oct-muted flex items-center gap-1">
                            {isTelegram
                            ...
                            <button
                              ...
                              className={`w-full text-left px-2.5 py-[5px] text-[13px] flex items-center gap-1.5 rounded-[3px] mx-0.5 transition-colors ${
                                ch.channelId === selectedChannelId
                                  ? 'bg-oct-live-dim text-oct-text'
                                  : 'text-oct-muted hover:bg-oct-surface-raised hover:text-oct-text'
                              }`}
            ...
            <button
              ...
              className={`flex items-center justify-center w-[34px] h-[34px] rounded transition-colors ${
                !sending && (content.trim() || files.length > 0) && effectiveChannelId
                  ? isTelegramChannel ? 'text-[#2AABEE] hover:text-[#5BC0F0]' : 'text-oct-live hover:text-[#3d5fff]'
                  : 'text-oct-muted/50 cursor-not-allowed'
              }`}
