...
import type { FomoStreamTrade } from '../types/fomoStream';
...
export function useWebSocket() {
  ...
  const addFomoStreamTrade = useAppStore((s) => s.addFomoStreamTrade);
    ...
    function connect() {
      ...
      ws.onmessage = (event) => {
        try {
          ...
          } else if (incoming.type === 'fomo_stream_trade') {
            // All-chain FOMO tape re-broadcast by 985monitor.xyz. Like the
            // Robinhood tape it is a public global feed from a third party, so
            // it lands in its own slice and raises no alert or sound: it is not
            // one of the user's tracked signals, it is not OCT's own
            // fomo.family feed, and it is never fused with either of those or
            // with convergence.
            addFomoStreamTrade(incoming.data as FomoStreamTrade);
  ...
  }, [addMessage, updateMessage, markMessageDeleted, addAlert, setConnected, updateReaction, addContract, enrichContract, updateContractChain, updateTokenPeak, fetchGuilds, fetchDMChannels, fetchHistory, fetchTelegramChats, checkAuth, setGatewayAuthError, fetchMaskedTokens, addFomoTrade, addRobinhoodFill, addFomoStreamTrade, addPumpCallout, addRevival, bumpJournalRefresh, bumpPriceAlertRefresh]);
}
