...
def train_market_policy(
    ...
    *,
    ...
    continuity); ``None`` builds a fresh larger-than-Phase-1 net. Requires the ``learn`` extra.
    """
    import random
...
def evaluate_rung(
    ...
    *,
    ...
    """Score the agent + the three baselines on the held-out tokens; assemble the rung report."""
...
def format_rung(result: RungResult) -> str:
    ...
    if result.evaluations:
        ...
        lines.append(header)
        for name in ("learned_agent", "hold_sol", "buy_and_hold", "random"):
    ...
    *,
    ...
    for rung in rungs:
        ...
        trained = train_market_policy(
            ...
            warm_start=warm if warm_start_across_rungs else None, log=log,
        )
        result = evaluate_rung(trained.policy, wf, config, n_tokens_requested=rung, seed=seed)
...
def _checkpoint(trained: TrainedPolicy, checkpoint_dir: Path, rung: int) -> None:  # pragma: no cover
    ...
    torch.save(trained.model.state_dict(), checkpoint_dir / f"rung_{rung}.pt")
