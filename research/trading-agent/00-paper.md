...
1. *Flow-tier encoder (§3.2, §5.2).* It is the Phase-B representation: rather than feeding Model N a raw bag of flow ratios, we feed the pretrained attention-state vector. Self-supervised pretraining on the swap firehose before any RL is exactly the representation-first / privileged-encoder move of §5.6, and its cross-token transfer directly addresses the small-per-token-data problem.
