...
- A POMDP formulation of new-pair memecoin trading with per-phase observation spaces, a concrete action space, and a **short-horizon episode definition** — the default archetype is a fast, high-volume scalper on freshly launched tokens (seconds-to-minutes), with population diversity permitting some longer-hold personalities — plus a principled critique of the 1→100 reward and a full reward-design treatment: a taxonomy of reward components, potential-based shaping as the anti-hacking discipline, and reward *composition* treated as an evolved trait searched by the population rather than a formula hand-tuned once (§3).
- A two-model system architecture (revival + new-pair) with a later independent-signal ensemble/convergence layer consistent with OCT's design principle that signals stay independent and are fused only at scoring (§4), and an explicit **signal-first commitment**: the primary deliverable is a calibrated decision emitted as a signal/API, with live autonomous execution an optional, separately-gated extension that never custodies funds by default — which removes most of the catastrophic-risk profile at the source (§4.2, §9.8).
- A **trade-flow attention subsystem** that measures a new pair's *endogenous* attention directly from its swap tape — a multivariate Hawkes process whose fitted buy-intensity is an interpretable *attention chart* and whose branching ratio is an attention-momentum scalar (n→1 = near-critical, a run dynamically possible), paired with a causal self-attention transformer as the learned, cross-token-transferable encoder (Hawkes as interpretable teacher, transformer as parent-facing student, their agreement a robustness signal) — serving simultaneously as the flow-tier encoder (§3.2, §5.2), an independent signal into the convergence layer (§4.3), and a standalone OCT alert, and always emitted with a mandatory manipulation-suspicion channel (§4.4; identification risk in §9.10; full treatment in the companion sub-project).
...
- **+ Wallet flows (Phase B):** the on-chain flow features derived from OCT's feed once wallets may be resolved — unique-buyer/seller counts, holder-count deltas, inflow from wallets tagged smart-money vs fresh/bot, concentration (top-holder share), and creator-wallet behavior. This is the first tier that reads *who* is trading, not just *how much*; several of these (creator dumping, concentration) are early rug-flow signatures. These flow features are consumed either as raw ratios or, preferably, through the **trade-flow attention encoder** (§4.4), which compresses the swap tape into a learned attention-state vector (see §5.2).
...
- **Per-token episodes** (default for early phases and for per-token benchmarking): an episode begins when a new pair enters the watchlist and runs to a *natural* terminal — full exit, token death, or liquidity below a floor — under a **single generous hard cap (~3 days)** that exists only to bound compute and guarantee termination, **not** to shape behavior. We deliberately do **not** impose a short maximum holding time. A hard seconds-to-minutes cap would force-sell precisely the runners the agent should hold, truncating the fat right tail that is the entire source of edge in this market; a max is a behavioral straitjacket, not a risk control. Instead the fast-scalper style is **learned, not mandated** — induced by the reward's holding/opportunity-cost and capital-efficiency terms (§3.5), so the agent discovers that rapid rotation is *usually* +EV yet can hold a specific token for as long as its expected value justifies. Consequently the meaningful statistic is **average holding time, which is a measured outcome, not a constraint** — far more informative than any imposed maximum — and it enters the design directly as a quality-diversity *descriptor axis* (§6.4, "holding time: scalper ↔ runner"), letting the population span fast scalpers through slower swing styles and retain the best performer in each hold-time niche rather than declaring one the default and the rest exceptions.
...
Reward design is where a project like this is quietly won or lost. The agent will optimize *exactly* what we score, with inhuman literalness and inexhaustible patience; every gap between what we write down and what we mean becomes a strategy. This section therefore does more than reject the naive goal. It (i) shows why "1 SOL → 100 SOL" is pathological *as a reward*, (ii) lays out the full **reward-design space** as a taxonomy of component classes with sharply different trust levels, (iii) states the two structural principles — anti-hacking through potential-based shaping, and treating reward *composition* as an evolved trait rather than a hand-tuned formula — that hold the whole design together, and (iv) bakes in one specific correction (never reward *unrealized* PnL). Throughout, the governing idea is that reward design here is not a formula to get right by hand but a **search over a deliberately constrained space** (§3.5.4, §6.4).

#### 3.5.1 The naive goal-as-reward is pathological
...
The fix is not a single better number. It is to compose the reward from a small library of components with *known* trust properties, keep the ungameable ones primary, and admit the gameable ones only in a form that provably cannot distort the optimum. The next three subsections make that concrete.

#### 3.5.2 The reward-design space: a taxonomy

We organize candidate reward terms into six classes, ordered from most to least trustworthy. The class boundaries are not cosmetic: they encode *how gameable* each term is, which in turn dictates whether it may be primary, shaping-only, or forbidden.

1. **North-star (evaluation only, never a training signal).** The 1→100 SOL trajectory is the scoreboard and nothing more (§3.5.1, §8.1). It is legible and it is the operator's actual success criterion, so we report it — but it never enters the gradient. It is a *report card, not a teacher*.

2. **Core P&L (dense, primary, ground truth).** Two terms anchor the reward. First, **percent return** on capital at risk, which normalizes across position sizes so the agent is rewarded for *quality* of decision rather than for sizing larger. Second, and decisively, **risk-adjusted PnL** — a differential Sharpe/Sortino-style ratio updated online (Moody & Saffell), a downside-deviation or CVaR penalty, or, equivalently, a risk-sensitive objective read straight off the distributional critic (§6.3, maximizing e.g. CVaR\(_\alpha\) rather than the mean). This is the term that *suppresses the martingale/lottery attractor* of §3.5.1: a policy that occasionally 100×'s and usually zeroes scores badly on a drawdown- and variance-aware measure even though it scores well on raw expectation. Realized, cost-inclusive, risk-adjusted PnL is the one class the agent cannot game without *actually making money at acceptable risk*; it is the ground truth against which every other term is a convenience or a hazard.

3. **Rank / benchmark-relative (the strongest "big" signal).** For each token the agent traded, reward its performance *relative to the labeled traders on that same token* — a percentile rank or top-\(N\) placement in the cohort's realized-PnL distribution on that token, controlling for entry/exit timing and size (§8.2). This term is powerful for two reasons that rarely coexist. It is **self-normalizing across market regimes**: beating the field is meaningful whether the field made or lost money on that token, so the signal stays informative through bull and bear metas without re-scaling — directly addressing the non-stationarity that §6.6 and §9.1 treat as the design premise. And it is **grounded**: unlike a pure shaping proxy, an agent cannot rank highly against real traders on real fills without genuine performance, so the term resists degenerate exploitation. It also operationalizes the operator's literal objective — "outperform these traders on every single token" — as a dense, per-token advantage rather than a sparse global balance. Along with class 2, this is a term that may be *primary*.

4. **Execution-quality proxies (secondary, shaping-only, individually gameable).** These reward the *texture* of good trading: early-entry timing (getting in before volume inflects), entry price versus VWAP, exit price versus the token's subsequent peak — the "buy low on average, sell high on average" intuition. They are genuinely useful for guiding early exploration, because they give dense feedback on the microstructure skills that class-2 PnL only rewards diffusely. But each is a **proxy, and each is individually gameable in isolation**: reward "average buy price low" and the agent learns to accumulate dying tokens whose price only ever falls; reward "got in early" and it apes *everything* early, since early entry is not the same as profitable entry; reward "sell near the peak" and it is tempted to sit on paper gains waiting for a peak that round-trips (§3.5.5). Because they are gameable, they must be **secondary and small**, and — per §3.5.3 — admitted only as potential-based shaping so they cannot move the optimum.

5. **Cost and behavioral terms (keep the policy honest and tradeable).** Realized trading is not free, and a reward that ignores its costs trains a strategy that only "works" on paper. This class includes: **holding/opportunity cost** (a mild per-step carry that makes idle capital cost something and thereby *induces*, rather than mandates, the fast-rotation scalper style — tying directly to the learned-not-imposed holding time of §3.4); realistic **transaction, fee, and slippage cost** charged on every fill (§6.2), without which the agent overtrades into oblivion; **concentration / exposure penalties** that discourage betting the book on one token; **capital-efficiency** (return per unit of time and capital deployed); and the small **information-cost** term of Phase D+ (a negative reward per web-search/tool invocation, so the agent learns *when* information is worth acquiring). These shape behavior toward strategies that survive contact with real costs.

6. **Penalties (hard-negative terms for the behaviors we refuse to teach).** A **severe** negative for buying into a rug or honeypot — an outcome we want the agent to treat as categorically different from an ordinary loss, since it is both avoidable and adversarial (§9.5); penalties for **churn** (excess turnover farming a mis-specified term) and for **breaching risk limits**. We distinguish penalties that *shape* from limits that *bind*: the truly catastrophic behaviors are not merely penalized but **constrained** (§3.5.5).

A design note on the no-trade baseline that cuts across the whole taxonomy: **holding SOL — doing nothing — is a legitimate and often-correct action**, so the composed reward must not implicitly punish inaction relative to a churn policy. We always evaluate against both a "hold SOL" and a "buy-and-hold the token" baseline (§8), and the cost terms of class 5 are calibrated so that patience is not silently taxed.

#### 3.5.3 Two load-bearing principles

Two principles govern how the taxonomy above is assembled into an actual reward. They are the intellectual core of this section.

**A. Reward hacking scales with the number of proxies — so proxies must be shaping, not objectives.** Every proxy term (class 4, and the softer parts of class 5) is a surface the agent can attack instead of the thing we meant. The more such terms we add, and the larger their weight, the more the reward becomes a specification of *loopholes* rather than of *goals* — this is the reward-hacking failure catalogued in §9.2. The design rule follows directly: the **primary reward must be realized, cost-inclusive, risk-adjusted PnL (class 2) and/or the rank-vs-traders signal (class 3)** — the terms that, by construction, cannot be maximized without genuinely making money at acceptable risk. Every proxy and shaping term must then be introduced as **potential-based reward shaping** (Ng, Harada & Russell, 1999): a shaping reward of the form \(F(s,s') = \gamma\,\Phi(s') - \Phi(s)\) for some potential function \(\Phi\) over states. This is the one shaping form *proven* to leave the optimal policy unchanged — it can accelerate and guide exploration, but it cannot create a new optimum, because its contribution telescopes to a constant over any complete trajectory. So "got in early" or "bought below VWAP" can *steer* the agent toward promising microstructure without ever making aping-everything-early or bag-holding-a-faller genuinely optimal. Proxies expressed any other way (as raw additive bonuses) are treated as design smells and are removed or re-expressed in potential form. This is the mechanism that lets us reap the sample-efficiency benefit of dense proxy feedback while structurally denying it the power to distort what the agent ultimately learns to want (cross-ref §9.2).

**B. Reward composition is an evolved trait, not a hand-tuned formula — which dissolves the "which reward is right?" problem.** There is no single correct set of weights across the taxonomy, and pretending otherwise invites months of manual tuning that overfits to whichever backtest window we happened to look at. We instead split the problem in two. The **human designs the component *library* and the hard guardrails** — the taxonomy of §3.5.2 plus the non-negotiable structure: PnL/rank primary (class 2/3), proxies potential-based (§3.5.3-A), ruin and rugs heavily penalized or constrained (class 6, §3.5.5). The **population/evolutionary search then searches the space of reward *weightings*.** §6.4 already lists reward weightings as an explicit Population-Based-Training dimension; this section supplies the reason it must be one. Different reward mixes are instantiated across the population, and only the mixes whose resulting policies show genuine, out-of-sample, cost-realistic edge (§8.5) survive selection — a reward composition that looks principled but breeds lottery-seekers or backtest-overfitters is culled by the same forward-validation that culls bad policies. This reframes reward design as *search over a constrained space* rather than a formula to be gotten right by hand, and it is also what gives the diverse-archetype deliverable (§6.4) its variety: different surviving personalities can rest on genuinely different reward emphases (a rank-chaser, a drawdown-averse grinder, a narrative-timer), each edge-positive in its own way. The guardrails keep the search inside the space of *honest* rewards; evolution finds the good points within it.

#### 3.5.4 One specific correction: never reward *unrealized* PnL

One error is common enough, and damaging enough, to call out on its own: **do not reward unrealized PnL, and never reward peak or highest-ever mark-to-market value.** A reward that credits paper gains — the value of a position at its momentary high — trains the agent to *hold for the paper peak*, and in a market where the overwhelming majority of new-pair spikes round-trip to zero, learning to sit on unrealized highs is learning to bag-hold. The agent would be positively reinforced for exactly the behavior that turns winners into losers: watching a position print a 10× on screen and refusing to realize it because the reward already paid out for the paper mark. All PnL terms (class 2) must therefore be computed on **realized** fills, and the risk-adjustment must penalize the round-trip: a position that reached a high and gave it back should score *worse* than one that took a smaller realized gain, not better. This mirrors — professionally, and worth stating because the whole project is partly an attempt to out-discipline human failure — one of the best-documented human trading failure modes: holding a peak too long and letting a realized win decay into a loss. We refuse to encode that reflex into the reward. (Class 4's "exit versus subsequent peak" proxy is the *bounded, potential-based* way to teach good exit timing without this pathology; the forbidden thing is rewarding the unrealized peak itself as objective value.)

#### 3.5.5 Hard constraints, and why 1→100 is retained

Some behaviors are too dangerous to leave to a weight in a reward sum. We therefore *constrain* rather than merely penalize the worst cases: hard per-token and per-session position caps and a max-drawdown circuit-breaker enter as **environment constraints / action masks**, not soft rewards — consistent with treating catastrophic risk as a safety layer, not a tunable trade-off (§9, §10). This is a constrained-MDP (CMDP) framing: maximize risk-adjusted return *subject to* hard risk constraints. The distinction from class-6 penalties matters — a penalty can always be outweighed by a large enough expected gain, which is precisely the loophole a 100×-seeking policy would exploit; a constraint cannot be traded away at any price, which is what catastrophic risk requires.

**Why keep 1→100 at all?** As an evaluation north-star it is honest and legible: it is the operator's success criterion and a single, interpretable headline (§8.1). But it lives in class 1 of the taxonomy for a reason — it is a *report card*, not a *teacher*. The training signal is the composed, guardrailed, evolved reward of §3.5.2–§3.5.3; the 1→100 run only ever grades the result.
...
**The primary deliverable is a signal, not a live trader.** This is a design decision, not a hedge. What Model N (and the attention model of §4.4) ships is a **typed, calibrated decision emitted as a signal into the convergence layer (§4.3) and, for consumers, as an API** — "this new pair is worth entering, at this size, with this confidence, for these reasons." A human, or the operator's existing tooling, acts on it. Live autonomous execution is an *optional, separately-gated extension* that OCT may never enable: it is strictly downstream of the signal, sits behind the paper→live promotion gate (§8.5) and the sniper safety envelope (§10.4), and nothing in the training program depends on it being switched on. Three reasons make signal-first the right default. (i) **It is the product with the widest surface and the least liability** — a ranked, explainable feed of igniting new pairs is directly useful in the console today and to third parties as an API, whereas a fund-holding autopilot is a narrow, high-stakes niche. (ii) **It removes most of the catastrophic-risk profile of §9.8 at the source**: a system that never custodies funds cannot be tilted into ruin, so the single largest danger in this whole program is opt-in rather than default. (iii) **It is measured the same way regardless** — a signal's edge is exactly the marginal convergence lift and the benchmark-relative rank (§8.2) that we would evaluate before ever letting it trade, so choosing signal-first costs the research nothing. The agent is trained to *decide well*; whether a decision is routed to a screen or to the sniper is a deployment switch, and the switch defaults to *screen*.

### 4.3 The ensemble / convergence layer

OCT already computes multi-signal convergence across independent signals: honest caller scoring, FOMO smart-money wallets, revival, and pump/KOL callouts. Model N becomes **one more independent signal** into that layer, and the trade-flow attention model (§4.4) is another. Crucially, per OCT's principle, we **do not fuse the underlying detections** — we fuse *scores*. The convergence layer takes each signal's calibrated output and combines them (e.g., a learned or rules-based scorer) into the console's ranking and, later, into any gated live action. This preserves interpretability (each signal can be inspected and disabled independently) and prevents a single model's failure from silently corrupting the others. It also means the RL agent's edge can be *measured marginally*: convergence-with-N vs convergence-without-N is a clean A/B.

### 4.4 The trade-flow attention model (flow-tier encoder and independent signal)

A third subsystem sits between the raw feed and both Model N and the convergence layer: a **trade-flow attention model** that infers a new pair's *endogenous attention* directly from its swap tape. Its condensed treatment follows; the full standalone development — extended related work, estimation detail, evaluation protocol, and a mini-roadmap — is the companion sub-project `subprojects/trade-flow-attention.md`.

**The idea.** A new-pair memecoin has no cash flows, no fundamentals, and almost no history; what it has is a crowd arriving or leaving, and its price is very nearly the derivative of that crowd's *attention*. Attention is thus not a sentiment overlay for this asset class but the closest thing to a valuation primitive it has. The academic way to measure investor attention is *exogenous* — Google search volume (Da, Engelberg & Gao, 2011), news counts, social mentions — but those proxies are coarse, lagged, hard to attribute to one contract among thousands sharing a meme, and for a token minutes old they usually do not exist yet. We instead measure attention **endogenously, from pure swap flow**: every buy is a revealed act of attention-plus-conviction, every sell attention-plus-exit, and the timing between trades, the acceleration of buys, and the widening unique-buyer set are the on-chain shadow of a crowd forming or dispersing. One correction is load-bearing: new pairs trade on **AMM bonding curves, not a central-limit order book**, so there is no resting book of bids and asks to read — this is a *trade-flow* attention model over the executed-swap stream, not an "orderbook" model. (The closed-form AMM impact map is a genuine advantage: it lets us cleanly separate "price moved because one whale traded" from "price moved because a crowd arrived," an attribution equity microstructure cannot make as sharply.)

**A dual backbone.** We estimate the attention state two ways and keep both.

- **A multivariate Hawkes process** (Hawkes, 1971; Bacry, Mastromatteo & Muzy, 2015) fit to the swap tape. Each event raises the intensity of near-future events, which is literally the self-reinforcing feedback loop of investor attention — a trade draws eyes, which draw trades — not a metaphor for it. The fitted buy-intensity **λ_buy(t) *is* the attention chart**: defined from the first trade, updated on every event, token-specific by construction. Its **branching ratio n** — the expected number of future trades one trade directly triggers — is a single interpretable **attention-momentum scalar**: n→0 means trades arrive only from exogenous discovery (attention is not compounding); n→1 means the process is **near-critical**, each trade nearly reproducing itself, so a small spark can sustain a cascade (a run is dynamically possible); n≥1 is explosive/unstable. This is exactly Filimonov & Sornette's (2012) market-*endogeneity* measure repurposed as the trader's question "is this thing actually catching, or is it just the deployer's bots?"
- **A causal (masked) self-attention transformer** over the swap sequence as the learned encoder. Here "attention" is the ML mechanism (Vaswani et al., 2017): per trade, learned weights decide which past trades are relevant to the current state — the neural operator (Sense B) estimating the crowd's cognitive state (Sense A). It relaxes the Hawkes functional form and, following the "universal microstructure features" result of DeepLOB (Zhang, Zohren & Roberts, 2019) and the LOB-transformer line, it **transfers across tokens**, which is how it survives the tiny per-token samples of the first minutes where Hawkes MLE is least stable. It produces the compact attention-state embedding the RL agent actually consumes.

The two are **teacher and student, not rivals.** The Hawkes model is the interpretable prior — its λ(t) and n are directly usable features, a self-supervision distillation target for the encoder, and the human-legible explanation when the encoder fires — while the transformer is the flexible, parent-facing representation that absorbs whatever the parametric form misses. Their **agreement is itself a robustness signal**, mirroring OCT's convergence philosophy (§4.3): when the parametric n and the learned score both say "igniting," the call is stronger than either alone; when they diverge, the divergence flags either manipulation or model misspecification.

**Pure-flow features.** Both backbones draw on the same causal, flow-only features: buy/sell velocity and acceleration (acceleration is the "igniting" signal); inter-trade time and its clustering; **order-flow imbalance** (Cont, Kukanov & Stoikov, 2014) in its AMM-exact, impact-adjusted form (the linear price-impact-in-signed-flow intuition traces to Kyle, 1985, but on a bonding curve the impact map is *known*, not estimated); **unique-buyer growth** — the breadth feature most aligned with genuine attention and least easily faked by one actor churning (buy-side flow is specifically the attention-carrying side, per Barber & Odean's, 2008, attention-driven-buying asymmetry); buyer **concentration** (top-holder share, Gini/Herfindahl); trade-size-distribution **Benford / round-number** tests (Cong et al.) as *inline* fake-flow features; and creator/sniper-wallet activity as a covariate that *down-weights* apparent attention.

**The mandatory authenticity channel.** The model's output is an attention state — { λ_buy(t), λ_sell(t), n(t), unique-buyer breadth, concentration, a **manipulation-suspicion score**, the learned embedding, and one calibrated attention score } — and it **never ships without the manipulation-suspicion channel**. The reason is structural and is developed in §9.10: wash trading manufactures the exact self-exciting tape the model reads as attention, so a high reading cannot be taken at face value.

**How it wires in.** The subsystem plugs into OCT at three points this paper already anticipates:

1. *Flow-tier encoder (§3.2, §5.2).* It is the Phase-B representation: rather than feeding Model N a raw bag of flow ratios, we feed the attention-state vector. Self-supervised pretraining on the swap firehose before any RL is exactly the representation-first / privileged-encoder move of §5.6, and its cross-token transfer directly addresses the small-per-token-data problem — but pretraining is a *warm start, not a freeze* (see **Codependent training** below).
2. *Independent convergence signal (§4.3).* Its calibrated attention score (with the authenticity channel) enters the convergence scorer as one more independent detector. Because it is derived *purely* from flow, it is mechanically independent of the social/caller signals — which is precisely what makes convergence between "flow says igniting" and "callers say igniting" meaningful rather than double-counting.
3. *Standalone alert.* "Attention is igniting on this pair" — a rising λ_buy with broadening unique buyers and a branching ratio climbing toward 1, *and a low manipulation-suspicion score* — is a shippable OCT alert in its own right, in the same family as revival and missed-runner, giving the subsystem a delivery path independent of the full agent landing.

**Codependent training (the attention model and the agent are trained side by side, not in sequence).** The three wire-in points above could read as a one-way pipeline — attention model pretrains, freezes, feeds the agent — and that is *not* the design. New pairs trade in an *attention market*: price is very nearly the derivative of the crowd's attention (§4.4 opening), so an agent trading here is trading *within* the attention system the model estimates, and the two are mutually dependent by construction. Concretely: (a) **the encoder is co-trained end-to-end with the policy.** After self-supervised pretraining it stays *unfrozen*; RL gradients from the agent's value/policy loss flow back into it alongside a standing self-supervised auxiliary loss (next-event / intensity prediction and Hawkes-distillation), so the representation is shaped both by "what reconstructs the tape" *and* by "what attention states actually precede profitable, risk-adjusted entries." The agent thus teaches the attention model *which* features of attention are decision-relevant, while the attention model gives the agent the only state worth reasoning over — a genuine two-way dependence, not a handoff. (b) **The coupling is reflexive, and that is exactly why joint training is required, not merely convenient:** the agent's own buys and sells add to λ_buy/λ_sell and inflate the very branching ratio *n* that would justify them (§9.10). An encoder frozen on passive-observer flow would systematically misread the tape the moment the agent starts acting on it; co-training on the agent's *own* interaction stream is what lets the policy learn its market impact on attention and discount it. (c) **The standalone signal (point 2/3) is preserved by architecture, not by separation:** a *stop-gradient copy* of the attention state — the self-supervised representation without the policy's task-shaping — feeds the convergence layer and the standalone alert, so the shared signal stays flow-derived and mechanically independent of the agent's objective even as the agent's private, co-trained view of attention specializes. The attention model can still be *born* alone (pretrained and shipped as an alert before the agent exists), but its mature form and the agent's mature form are one jointly-optimized system with two heads. *(Training-mechanics detail — auxiliary-loss weighting, when to unfreeze, the stop-gradient boundary, and the reflexive-impact correction — lives in the companion sub-project and is a Phase-1/2 experimental choice.)*
...
**Encoder:** this tier is represented not as a raw bag of ratios but through the **trade-flow attention encoder (§4.4)** — a pretrained, cross-token attention-state vector (Hawkes buy-intensity + branching ratio + a learned self-attention embedding, with an authenticity channel). Whether the *learned* encoder beats hand-built flow ratios is itself one of the tier's ablations (§8.4).
...
### 9.10 The identification limit of flow-derived attention (the attention model)

The trade-flow attention model (§4.4) carries a threat severe enough to be structural, not incidental: **self-excitation is not identifiable from a common exogenous driver using flow alone.** A burst of clustered buys is equally consistent with (a) genuine endogenous attention — a real crowd feeding on itself — and (b) a hidden common cause — one piece of off-chain news, one influencer, or one coordinated actor — driving many arrivals that merely *look* self-exciting. The Hawkes decomposition into an exogenous base rate μ(t) and the self-exciting term is notoriously fragile: a rising μ(t) and a high branching ratio n are near-substitutes in the likelihood, so the same tape can be attributed to "lots of news" or "critical self-excitation" almost interchangeably (Filimonov & Sornette, 2012, treat exactly this endogeneity estimate as the object of interest, and the near-criticality it reports can itself be a calibration artifact). **Consequence: a high attention reading cannot, from flow alone, be decomposed into genuine crowd attention versus a single driver or a manufactured cascade.**

Manipulation makes this the base case rather than a tail risk. **Wash trading manufactures precisely the clustered, self-exciting signature the model reads as attention** — a deployer can synthesize a high branching ratio to fake ignition — and Cong, Li, Tang & Yang (*Crypto Wash Trading*) estimate wash trading has averaged **>70% of reported volume** on unregulated venues; this is the single quantitative figure this paper cites, and only as motivating context. Sybil buyers defeat even the breadth feature (unique-buyer growth) by splitting one entity across many fresh wallets, and Lillo & Farmer's (2004) order-splitting result shows the microstructure of one actor splitting orders is genuinely hard to distinguish from many independent buyers — the same long-memory sign signature. This is a fundamental limit of what flow can reveal, not a bug to be engineered away.

The honest posture, which the subsystem is built around, is therefore: **the attention chart measures clustered flow *intensity*; its interpretation as genuine crowd attention is a corroboration-dependent hypothesis, never an identity.** Corroboration comes from unique-buyer breadth, wallet-age/funding-graph checks, cross-signal convergence (§4.3), and the mandatory manipulation-suspicion channel — which is why the model **never ships without that authenticity channel** (§4.4). Two further caveats travel with it (developed in the companion sub-project): even *perfectly* measured attention is not edge — Da et al. (2011) show attention leads a run-up *and then a reversal*, so trading it naively buys the top; attention is a driver of both gains and losses, an input to the risk-adjusted reward, not a decision — and at any non-trivial scale the signal is **reflexive**, since OCT's own buys add to λ_buy and inflate the very branching ratio used to justify them, so own-flow must be modeled and subtracted and the standalone-vs-at-scale gap monitored like the paper→live fidelity meter of §8.5.

### 9.11 Feasibility honesty
...
- Ng, Harada & Russell (1999). *Policy Invariance Under Reward Transformations: Theory and Application to Reward Shaping* (potential-based reward shaping).
...
- Hawkes (1971). *Spectra of Some Self-Exciting and Mutually Exciting Point Processes* (self-exciting point processes; the trade-flow attention backbone, §4.4).
- Kyle (1985). *Continuous Auctions and Insider Trading* (linear price impact of order flow).
- Lillo & Farmer (2004). *The Long Memory of the Efficient Market* (order-sign long memory; order-splitting — the sybil-indistinguishability point, §9.10).
- Barber & Odean (2008). *All That Glitters* (attention-driven buying; the buy-side asymmetry).
- Da, Engelberg & Gao (2011). *In Search of Attention* (search-volume attention proxy; attention leads a run-up then a reversal).
- Filimonov & Sornette (2012). *Quantifying Reflexivity in Financial Markets* (Hawkes branching ratio as a market-endogeneity / near-criticality measure).
- Cont, Kukanov & Stoikov (2014). *The Price Impact of Order Book Events* (order-flow imbalance).
- Bacry, Mastromatteo & Muzy (2015). *Hawkes Processes in Finance* (survey; primary Hawkes-in-finance methodological reference).
